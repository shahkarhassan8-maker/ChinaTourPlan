...
export function generateItineraryPDF({ itinerary, formData, title }) {
    ...
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text('Need Help? Contact Us', pageWidth / 2, yPos + 7, { align: 'center' });

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text('WeChat: Shahkarhassan | WhatsApp Available', pageWidth / 2, yPos + 13, { align: 'center' });

    doc.setTextColor(...grayColor);
    doc.text(`Generated by ChinaTourPlan - ${new Date().toLocaleDateString()}`, pageWidth / 2, yPos + 18, { align: 'center' });
