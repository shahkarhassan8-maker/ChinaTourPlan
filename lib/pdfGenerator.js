...
export function generateItineraryPDF({ itinerary, formData, title }) {
    if (!itinerary || !Array.isArray(itinerary) || itinerary.length === 0) {
        throw new Error('Invalid itinerary data: itinerary must be a non-empty array');
    }
...
export function downloadItineraryPDF({ itinerary, formData, title }) {
    try {
        if (!itinerary || !Array.isArray(itinerary) || itinerary.length === 0) {
            console.error('PDF Generation Error: Invalid itinerary data', itinerary);
            throw new Error('No itinerary data available to generate PDF');
        }

        const doc = generateItineraryPDF({ itinerary, formData, title });
        const cityNames = [...new Set(itinerary.map(d => d.city).filter(Boolean))].join('-') || 'China';
